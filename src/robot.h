...
class Robot : public ArenaMobileEntity {
  ...
  */
  ...
  /**
   * @brief Returns how many lives the robot has left. Game is over when lives_
   * is 0.
   */
  int get_lives() { return lives_; }

  /**
   * @brief Set the robot's lives_. Only needed in two cases.
   * CASE 1: Setting the initial lives to 9.
   * CASE 2: Subtracting a life upon a collision.
   */
  ...
  /**
   * @brief Called when game starts or is unpaused to get the robot moving at the
   * last speed.
   */
  void ResumeSpeed() override;

  /**
   * @brief Called when the game pauses, save the current velocity of the robot forward
