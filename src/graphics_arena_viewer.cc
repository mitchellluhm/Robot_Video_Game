...
void GraphicsArenaViewer::UpdateSimulation(double dt) {
  // Only update simulation if game has not ended
  if (arena_->get_game_status() == PLAYING) {
    controller_->AdvanceTime(dt);
  } else if (arena_->get_game_status() == WON) {
    // Notify user they have won
    nvgFontSize(screen()->nvgContext(), 70.0f);
    nvgTextBox(screen()->nvgContext(), 300, 300, 500, "Winner", NULL);
    // Wait for user to quit or start new game
  } else {
    // Notify user they have lost
    nvgFontSize(screen()->nvgContext(), 70.0f);
    nvgTextBox(screen()->nvgContext(), 300, 300, 500, "Loser", NULL);
    // Wait for user to quit or start new game
  }
}
...
void GraphicsArenaViewer::OnPlayingBtnPressed() {
  ...
  if (!paused_) {
    ...
    // Just Starting or Paused --> Playing
    playing_button_->setCaption("Playing");
    arena_->Resume();
  }
  paused_ = !paused_;  // swap the playing state
}

void GraphicsArenaViewer::OnNewGameBtnPressed() {
  std::cout << "NEW GAME BTN PRESSED" << std::endl;
  controller_->ConvertComm(kNewGame);
}
...
void GraphicsArenaViewer::OnSpecialKeyDown(int key,
  __unused int scancode, __unused int modifiers) {
    ...
    // int key : integer that represents which key pressed
    // Switch through special keys of interest, set their types
    switch (key) {
    case GLFW_KEY_LEFT:
      key_value = kKeyLeft;
      break;
...
void GraphicsArenaViewer::ResetCommonGraphics() {
  paused_ = true;
  playing_button_->setCaption("Start Game");
}
