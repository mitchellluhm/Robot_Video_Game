...
class Arena {
 public:
  ...
  /**
   * @brief Accepts a movement or change game state command from controller
   */
  ...
  **/
  ...
  /**
   * @brief delete all entities in entities_ vector and clear the
   * entities_ and mobile_entities_ vectors
   */
  void CleanArena();

  /**
   * @brief Calls ResumeSpeed() on all mobile entities. This is used
   * to get the entities moving when game starts or game is resumed
   */
  void Resume();

  /**
   * @brief Calls PauseSpeed() on all mobile entities. This is used
   * to get the entities to stop moving when user pauses the game.
   */
  void Pause();

  /**
   * @brief Kill the current factory when new game invoked.
   */
  void DeleteFactory() {
    delete factory_;
  }
  ...
  /**
   * @brief Creates a new EntityFactory, put game_status_ to playing,
   * and recreate needed entities (very similar to Arena's constructor).
   *
   * @TODO Make this use params specs rather than hardcoded values.
   */
  void ReconstructArena();
