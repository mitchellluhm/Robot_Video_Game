...
void Arena::AddEntity(EntityType type, int quantity) {
  for (int i = 0; i < quantity; i++) {
    if (type == kObstacle) {
      std::cout << "An obstacle was added" << std::endl;
      Obstacle *new_obstacle =
        dynamic_cast<Obstacle *>(factory_->CreateEntity(type));
      entities_.push_back(new_obstacle);
...
void Arena::UpdateEntitiesTimestep() {
  ...
  // Check if all bases captured
  int bases = 0;
  int bases_captured = 0;
  for (auto &ent : entities_) {
  ...
  if (bases > 0 && bases == bases_captured) {
    game_status_ = WON;  // win if all bases captured
  }
  // END check if all bases captured
...
void Arena::AdjustEntityOverlap(ArenaMobileEntity * const mobile_e,
  ArenaEntity *const other_e) {
    ...
    double delta_y = other_e->get_pose().y - mobile_e->get_pose().y;
    double distance_between = sqrt(delta_x*delta_x + delta_y*delta_y);
    bool is_left = delta_x > 0;
    double distance_to_move =
      mobile_e->get_radius() + other_e->get_radius() - distance_between + 6;
    if (is_left) {
...
void Arena::AcceptCommand(Communication com) {
  ...
  case(kTurnRight): robot_->TurnRight();
    break;
  case(kPlay):  // GET PREVIOUSLY MOVING ENTITIES MOVING AGAIN
    break;
  case(kPause):
    break;
  case(kReset):  // Wipe out everything in arena
      break;
  case(kNone):
    break;
  default: break;
...
void Arena::Resume() {
  // go through all mobile entities and call ResumeSpeed()
